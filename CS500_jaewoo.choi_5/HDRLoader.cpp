...
bool HDRLoader::load(const char* fileName, HDRLoaderResult& res)
{
    ...
    //cols -= h * w * 3;
 //   res.image = new float[h * w * 3];
 //   for (int i = 0; i < w * h * 3; i+=3)
 //   {
	//res.image[i + 0] = cols[i];
	//res.image[i + 1] = cols[i+1];
	//res.image[i + 2] = cols[i+2];
 //   }
    res.pBuffer = new float[res.width * (res.height + 1)];
    res.pUDist = &res.pBuffer[res.width * res.height];
    float* pSinTheta = new float[res.height];
    float angleFrac = PI / float(res.height);
    float theta = angleFrac * 0.5f;
    for (unsigned int i = 0; i < res.height; i++, theta += angleFrac)
	pSinTheta[i] = sin(theta);
    for (unsigned int i = 0, m = 0; i < res.width; i++, m += res.height) {
	float* pVDist = &res.pBuffer[m];
	unsigned int k = i * 3;
	pVDist[0] = res.cols[k + 0] * 0.2126f + res.cols[k + 1] * 0.7152f + res.cols[k + 2] * 0.0722f;
	pVDist[0] *= pSinTheta[0];
	for (unsigned int j = 1, k = (res.width + i) * 3; j < res.height; j++, k += res.width * 3) {
	    float lum = res.cols[k + 0] * 0.2126f + res.cols[k + 1] * 0.7152f + res.cols[k + 2] * 0.0722f;
	    pVDist[j] = pVDist[j - 1] + lum * pSinTheta[j];
	}
	if (i == 0)
	    res.pUDist[i] = pVDist[res.height - 1];
	else
	    res.pUDist[i] = res.pUDist[i - 1] + pVDist[res.height - 1];
    }
